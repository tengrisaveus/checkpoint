      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8">
          {getCoverUrl(game) ? (
            ...
            <div className="w-64 h-80 bg-gray-700 rounded-lg flex items-center justify-center text-gray-500">No Cover</div>
          )}
          ...
          <div className="flex-1">
            ...
            {game.platforms && <p className="text-gray-500 text-sm mt-1">{game.platforms.map((p) => p.name).join(", ")}</p>}
            {game.involved_companies && <p className="text-gray-500 text-sm mt-1">{game.involved_companies.map((c) => c.company.name).join(", ")}</p>}
            {game.rating && <p className="text-yellow-400 mt-3">IGDB Rating: {game.rating.toFixed(1)}</p>}
            {game.summary && <p className="text-gray-300 mt-4">{game.summary}</p>}
          </div>
        </div>

        {user && (
          <div className="bg-gray-800 rounded-lg p-6 mt-8">
            <h2 className="text-xl font-semibold text-white mb-4">Add to Library</h2>

            {success && <p className="bg-green-500/20 text-green-400 p-3 rounded mb-4">{success}</p>}
